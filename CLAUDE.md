...
- **證道筆記匯入／匯出**：`services/sermon_notes_io.dart`（`sermonNotesToText`／
  ...
  經文/分類/筆記/祂的話/實踐/感想` 小標，`---` 分隔）。「祂的話」的**分類自填**（存
  `trinity_who`，非固定選項；編輯頁四個位格只是快速點選捷徑）。匯入相容舊標籤「位格」。
  檔案存取走 `download_web`/`download_stub` 條件式 import（`downloadTextFile`／
  `pickTextFile`／`canPickFile`；手機無選檔→貼上文字）。⛔ 只做格式轉換，不代寫內容。
